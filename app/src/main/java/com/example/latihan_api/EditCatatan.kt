...
class EditCatatan : AppCompatActivity() {
    ...
    fun setupEvents() {
        binding.tombolEdit.setOnClickListener {
            ...
            val user_id = intent.getIntExtra("id_catatan",0)

            if (isi.isEmpty() || judul.isEmpty()) {
                displayMessage("judul dan isi catatan belum diisi")
                return@setOnClickListener
            }

            lifecycleScope.launch {
                val catatan = Catatan(id, judul, isi, user_id)
